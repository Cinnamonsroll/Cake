...
module.exports = {
  async cache(client, message, updating) {
    if (
      "tags" in client.cakeCache &&
      client.cakeCache.tags[message.guild.id] &&
      !updating
    )
      return;
    else {
      client.cakeCache.set("tags", {
        type: "dict",
        sub: {
          name: message.guild.id,
          type: "list",
          value: await client.tags.getTags(message),
        },
      });
    }
  },
  ...
  async findTag(client, query, guild) {
    let tag = client.cakeCache.tags[guild].find(
      (x) =>
